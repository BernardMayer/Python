...
with open(dsxFilename, 'r') as fDsx :
    for line in fDsx.readlines() :
        ...
        if (bDsjob and not bInitialize and line[0:10] == 'mainloop {') :
            ...
            dDsx[dsjobIdentifier]['dsrecords'][dsrecordName]["err_code"] = list()
            dDsx[dsjobIdentifier]['dsrecords'][dsrecordName]["err_desc"] = list()      
            if (bShowAll and bShowStructure) :
        ...
        if (line.find("if (") > -1) :
            ...
            #print("ifBloc :", ifBloc)
            # dDsx[dsjobIdentifier]['ifBloc'] = dict()
            # dDsx[dsjobIdentifier]['ifBloc'][ifBloc] = dict()
            # dDsx[dsjobIdentifier]['ifBloc'][ifBloc]["err_code"] = list()
            # dDsx[dsjobIdentifier]['ifBloc'][ifBloc]["err_desc"] = list()                  
        ...
        if (bMainloop and posErrCode > 0 and posErrCode < posEqual) : 
            ...
            # dsrecordIfBloc = ifBloc.replace("(", "").replace(")", "")
            # dsrecordIfBloc = dsrecordIfBloc.replace("&&", "")
            # dsrecordIfBloc = dsrecordIfBloc.replace("!", "")
            # dsrecordIfBloc = dsrecordIfBloc.replace("trimc_string", "")
            # dsrecordIfBloc = dsrecordIfBloc.replace("StageVar0_", "")
            ...
            if (retCode) : 
                ...
                s = s.replace(" ", "")
                #dDsx[dsjobIdentifier]["dsrecords"][dsrecordName]["err_code"].append("+".join(l))
                dDsx[dsjobIdentifier]["dsrecords"][dsrecordName]["err_code"].append("[" + dsrecordIfBloc + "]" + TAB + s)
                #dDsx[dsjobIdentifier]["ifBloc"][dsrecordIfBloc]["err_code"].append("[" + dsrecordIfBloc + "]" + TAB + s)
                pass
            else :
                print(lineNumber, "Pb:", "+".join(l))
            if (bShowAll and bShowErrCode) :
                print(lineNumber, "ERR_Code :", line.strip(), TAB, " dsrecordName :", dsrecordName, " ifBloc :", ifBloc) 
            continue
        ...
        if (bMainloop and posErrDesc > 0 and posErrDesc < posEqual) : 
            ...
            if (retCode) : 
                # TODO
                (retCode, s) = replaceIntervarByValues(l, dIntervar, dJobObjectVersion, dDsx[dsjobIdentifier]['dsjobName'], dDsx[dsjobIdentifier]['dsjobVersion'])
                #dDsx[dsjobIdentifier]["dsrecords"][dsrecordName]["err_desc"].append("+".join(l))
                dDsx[dsjobIdentifier]["dsrecords"][dsrecordName]["err_desc"].append("[" + dsrecordIfBloc + "]" + TAB + s)
                #dDsx[dsjobIdentifier]["ifBloc"][dsrecordIfBloc]["err_desc"].append("[" + dsrecordIfBloc + "]" + TAB + s)
                pass
            else :
                print(lineNumber, "Pb:", "+".join(l))
            if (bShowAll and bShowErrDescription) :
                print(lineNumber, "ERR_Desc :", line.strip(), TAB, " dsrecordName :", dsrecordName, " ifBloc :", ifBloc) 
            continue
...
for v in dDsx : 
    if (dDsx[v]["dsjobName"]) : 
        sLineOut = dDsx[v]["dsjobName"] + TAB + dDsx[v]["dsjobVersion"] + TAB + dDsx[v]["dsjobModifDate"] + TAB 
        if (dDsx[v]["dsjobPlanif"]) :
            sLineOut += "au plan" + TAB 
        else :
            sLineOut += "inactif" + TAB 
        ##  
        for dsrecord in dDsx[v]["dsrecords"] : 
            #sLineOut += ifBloc + TAB
            for errCode in dDsx[v]["dsrecords"][dsrecord]["err_code"] :
                print(sLineOut + "ERR_Code" + TAB + errCode)
            for errDesc in dDsx[v]["dsrecords"][dsrecord]["err_desc"] :
                print(sLineOut + "ERR_Desc" + TAB + errDesc)
